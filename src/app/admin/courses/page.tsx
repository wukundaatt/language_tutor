...
function CoursesContent() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<CourseItem | null>(null);
  ...
  const loadAll = () => {
    ...
    Promise.all([
      ...
      .then(([coursesData, langData]) => {
        let courseList: CourseItem[] = coursesData.courses || [];
        if (search.trim()) {
          const q = search.toLowerCase();
          courseList = courseList.filter(
            (c) =>
              c.title.toLowerCase().includes(q) ||
              c.language_name?.toLowerCase().includes(q) ||
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editMode && currentId !== null) {
        const res = await fetch(`/api/admin/courses/${currentId}`, {
          method: 'PATCH',
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/admin/courses/${confirmDelete.id}`, {
        method: 'DELETE',
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
  ...
  const columns = [
      ...
      render: (row: CourseItem) => (
        <div className="flex items-center gap-2">
          <div
            ...
            style={{ backgroundColor: row.cover_color }}
          >
    ...
    },
    {
      ...
      label: '单元',
      render: (row: CourseItem) => (
        <span className="text-sm text-[var(--foreground)]">{row.unit_count}</span>
      ),
    },
    {
      key: 'lesson_count',
      label: '课时',
      render: (row: CourseItem) => (
        <span className="text-sm text-[var(--accent)] font-medium">{row.lesson_count}</span>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[var(--foreground)]">课程管理</h1>
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <FormField label="封面颜色">
            <input
              type="color"
              value={form.cover_color}
              onChange={(e) => setForm({ ...form, cover_color: e.target.value })}
              className="w-full h-10 bg-[var(--background)] border border-[var(--card-border)] rounded-xl cursor-pointer"
            />
          </FormField>
        </div>
        <FormField label="排序">
        ...
        <div className="py-2">
          <div className="flex items-start gap-3 p-4 rounded-xl bg-rose-500/10 border border-rose-500/20">
            ...
            <div>
              ...
              确定要删除课程 <span className="text-rose-400 font-bold">{confirmDelete?.title}</span> 吗？
            </p>
              <p className="text-xs text-[var(--muted)] mt-1">
                这将删除该课程下的所有单元、课时及其相关学习记录，且不可恢复。
              </p>
            </div>
          </div>
        </div>
